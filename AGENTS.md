...
| `living_visual.py` | Phase 21 Living Visual World runtime: `ArtMoment`/`VisualJob` models, deterministic Art Director eligibility (`evaluate_art_moments`), the generate → quarantine → Guardian → approval job lifecycle, human creative-direction extension point, and query surfaces. |
| `living_visual_provider.py` | Phase 21 unified visual provider runtime over the existing `app.comfyui` client/workflow stack (configurable URL, workflow-role selection by visual type, timeout/retry, health/capability diagnostics, deterministic mock fallback). |
| `living_visual_compiler.py` | Phase 21 structured `VisualSceneSpecModel` compiler separating locked canonical facts, prohibited additions, unknown fields, interpretation fields, and source evidence. |
...
| `temporal.py` | Phase 22 Temporal Pacing & Living Time: injectable trusted clock, `TemporalContext`, distinct time domains, wall-clock/hybrid cooldowns, fictional-time policy, provenance-backed scheduled consequences, and return recap. Domain systems consume this instead of calling the system clock ad hoc. |
...
24. **SoulSmith does not illustrate everything; it paints what becomes worth remembering, and memory is canon while art is interpretation.** Phase 21 (`living_visual.py`, `living_visual_provider.py`, `living_visual_compiler.py`, plus the `art_moments`/`visual_jobs`/`visual_job_guardian_reports`/`visual_curations` tables in `db.py`) turns the completed visual architecture into a living loop: `CANONICAL STATE -> ART DIRECTOR ELIGIBILITY -> SCENE/PORTRAIT/OBJECT SPEC -> VISUAL PROVIDER -> QUARANTINE -> VISUAL CANON GUARDIAN -> APPROVAL -> GALLERY/MEMORY`, never `GENERATED IMAGE -> NEW CANON`. `evaluate_art_moments` is deterministic and selective (most events produce no Art Moment); `visual_jobs` persist a resumable, auditable generation lifecycle (provider/workflow/version/reference assets/Guardian verdicts); portrait continuity keeps locked historical references and never silently falls back to text-to-image; the Visual Canon Guardian stays between quarantined output and player-visible approval; provider failure never damages campaign state; and the deterministic mock path works without GPU/ComfyUI. Human creative direction (`/api/v1/living-visuals/art-moments/{id}/curate`) may interpret style/composition/mood/motif but never rewrite locked canonical facts. See `docs/LIVING_VISUAL_WORLD.md`.
25. **Time may change what is possible; it must never decide what the player's story means, and the world may wait without punishing a life outside the game.** Phase 22 (`temporal.py`, plus the `campaign_time_settings`/`aspect_temporal_state`/`temporal_cooldowns`/`scheduled_consequences` tables and `seeds.last_echo_at`/`promises.deadline_*`/`relationships.last_interaction_at`/`places.last_visited_at` columns in `db.py`) adds an injectable trusted clock (`freeze_time`/`now_utc`) and a bounded `TemporalContext` that separate canonical event time, server wall-clock, session time, elapsed real time, fictional time, Aspect-relative time, and place/history time. Count-based pacing (`COOLDOWN_WINDOW`) stays authoritative for offline fixtures; optional wall-clock cooldowns (`WALL_CLOCK_COOLDOWN_SECONDS`, empty by default) and hybrid rules are an *additional* eligibility gate, never a substitute for domain evidence. Fictional time is an explicit opt-in policy (`none` default); scheduled consequences must cite their originating evidence; each Aspect keeps independent temporal state across switches; and `resume-recap` is a gentle memory service, never a retention trick. The narrative Guardian also rejects `invented_temporal_consequence`. See `docs/TEMPORAL_PACING_AND_LIVING_TIME.md`.
...
- Living Visual World & ComfyUI runtime → `backend/app/living_visual.py`, `backend/app/living_visual_provider.py`, `backend/app/living_visual_compiler.py`, `docs/LIVING_VISUAL_WORLD.md`
- Temporal Pacing & Living Time → `backend/app/temporal.py`, `backend/app/campaign_orchestrator.py`, `docs/TEMPORAL_PACING_AND_LIVING_TIME.md`
