...
def generate_astrological_soulprint(req: SoulprintRequest) -> SoulprintProfile:
    if not req.user_consent:
        raise ValueError("Explicit GDPR consent is required to generate an Astrological Soulprint.")
    ...
    date_parts = [int(p) for p in req.birth_date.split("-")] if "-" in req.birth_date else [1995, 6, 15]
