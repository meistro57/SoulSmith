...
def evaluate_scene_outcome(req: ResolveSceneRequest) -> ResolveSceneResponse:
    base_verdict = req.dice_read.verdict
    ...
    approach_match = (req.chosen_approach.lower() == req.dice_read.approach.lower())
    ...
    if score >= 4:
        ...
        title = "Ascendancy"
        res_delta = -req.resonance_spent + (1 if req.dice_read.spark == "Wonder" else 0)
    ...
    canon_list = [fact]
    if fracture:
        canon_list.append(f"CRITICAL: {req.soul_name}'s Strain reached maximum (6/6). A Soul Fracture has occurred!")
    if req.dice_read.thread in ["Bond", "Memory", "Mark", "Debt", "Portal", "Prophecy"]:
        canon_list.append(f"A lasting [{req.dice_read.thread}] Thread has been recorded into the World Chronicle.")
    ...
    return ResolveSceneResponse(
        ...
        new_resources=SoulSheetResources(resonance=new_res, strain=new_str, thread_count=new_thr),
