import type { AlternateSceneResult, ArtDirectionProfile, ArtDirectionProfileVersion, ArtDirectionSpec, ArtifactType, ArtMoment, Aspect, AspectKnowledgeProjection, AspectSwitchResult, AuthResponse, AvatarIdentity, AwakeningStage, Biography, BiographyGuardianReport, BiographySection, BiographySourceRef, CampaignAspect, CampaignOpportunity, CampaignSession, CampaignTimeSettings, CampaignTransition, CanonStatus, CanonicalDiceRead, ChroniclePainting, CommunitySymbol, Constellation, ConstellationStageInfo, CrossAspectBond, CrossAspectMeeting, EncounterFrame, GalleryArtifact, GalleryCollection, GalleryCollectionItem, GatheringContribution, GatheringSession, GenerationType, GroupAnchorType, GroupMemory, GroupMemoryAnchor, GroupMemoryTag, GroupTagType, HumanCuration, IntegrationEvent, LegendaryFigure, LocalThread, LocationConsent, ManifestationType, MemoryObject, NarrativeGeneration, NarrativeIntensity, NearbyDiscoveryResult, NPCKnowledgeProjection, NumericDiceRoll, OpenQuestion, ParticipantRef, PerspectiveComparison, Place, PlaceHistoryEntry, PlayerPreferences, PortraitGenerationCandidate, PortraitVersion, PrivateNote, ProbablePath, ProbablePathStatus, PromptCompilationResult, PromiseParticipant, PromiseRecord, ProviderCapabilities, RecognitionDecision, ReflectionSession, RelatedMemorySuggestion, Relationship, RelationshipParticipant, RelationshipPromiseKnowledgeProjection, Relic, RelicEvent, ResolvedArtDirection, ResolvedScene, ReturnRecap, ScheduledConsequence, Seed, SoulResources, SoulprintProfile, SpiritualFraming, StoryMark, StyleReviewResult, TemporalContext, TemporalEligibility, User, VisualAvatarProfile, VisualEntityVersion, VisualJob, WorldEntityType, WorldMemory, WorldMemoryPlacement, WorldVisualCandidate } from '../types';
...
export const apiClient = {
  ...
  createRelationship: (payload: { kinds?: string[]; participants: RelationshipParticipant[]; source_type?: string; source_id?: string; creation_context?: string; visibility?: string; status?: string }) => request<{ relationship: Relationship }>('/api/v1/relationships', { method: 'POST', body: JSON.stringify(payload) }),
  listRelationships: (params?: { viewer_soul_id?: string; entity_type?: string; entity_id?: string }) => request<{ relationships: Relationship[] }>(`/api/v1/relationships?${new URLSearchParams((params ?? {}) as Record<string, string>).toString()}`),
  getRelationship: (relationshipId: string, viewerSoulId?: string) => request<{ relationship: Relationship }>(`/api/v1/relationships/${encodeURIComponent(relationshipId)}${viewerSoulId ? `?viewer_soul_id=${encodeURIComponent(viewerSoulId)}` : ''}`),
  addRelationshipEvent: (relationshipId: string, payload: { event_type: string; source_type: string; source_id: string; summary?: string }) => request<{ relationship: Relationship }>(`/api/v1/relationships/${encodeURIComponent(relationshipId)}/events`, { method: 'POST', body: JSON.stringify(payload) }),
  addRelationshipPerspective: (relationshipId: string, payload: { entity_type: string; entity_id: string; kind: string; view?: string; is_canonical_interaction?: boolean; visibility?: string }) => request<{ relationship: Relationship }>(`/api/v1/relationships/${encodeURIComponent(relationshipId)}/perspectives`, { method: 'POST', body: JSON.stringify(payload) }),
  createPromise: (payload: { promisor_entity_type: string; promisor_entity_id: string; promise_text: string; structured_meaning?: Record<string, any>; conditions?: string[]; scope?: string; visibility?: string; source_type: string; source_id: string; source_authorization?: string; participants?: PromiseParticipant[]; inheritable?: boolean; transferable?: boolean }) => request<{ promise: PromiseRecord }>('/api/v1/promises', { method: 'POST', body: JSON.stringify(payload) }),
  listPromises: (params?: { viewer_soul_id?: string; entity_type?: string; entity_id?: string }) => request<{ promises: PromiseRecord[] }>(`/api/v1/promises?${new URLSearchParams((params ?? {}) as Record<string, string>).toString()}`),
  getPromise: (promiseId: string, viewerSoulId?: string) => request<{ promise: PromiseRecord }>(`/api/v1/promises/${encodeURIComponent(promiseId)}${viewerSoulId ? `?viewer_soul_id=${encodeURIComponent(viewerSoulId)}` : ''}`),
  transitionPromise: (promiseId: string, payload: { new_state: string; evidence_type?: string; evidence_id?: string; player_authorized?: boolean; reason?: string; target_entity_type?: string; target_entity_id?: string }) => request<{ promise: PromiseRecord; message: string }>(`/api/v1/promises/${encodeURIComponent(promiseId)}/transition`, { method: 'POST', body: JSON.stringify(payload) }),
  linkPromiseEntity: (promiseId: string, payload: { link_type: string; entity_type: string; entity_id: string }) => request<{ promise: PromiseRecord }>(`/api/v1/promises/${encodeURIComponent(promiseId)}/links`, { method: 'POST', body: JSON.stringify(payload) }),
  queryRelationships: (entityType: string, entityId: string, viewerSoulId?: string) => request<{ relationships: Relationship[] }>(`/api/v1/relationships/query?entity_type=${encodeURIComponent(entityType)}&entity_id=${encodeURIComponent(entityId)}${viewerSoulId ? `&viewer_soul_id=${encodeURIComponent(viewerSoulId)}` : ''}`),
  queryPromises: (entityType: string, entityId: string, viewerSoulId?: string) => request<{ promises: PromiseRecord[] }>(`/api/v1/promises/query?entity_type=${encodeURIComponent(entityType)}&entity_id=${encodeURIComponent(entityId)}${viewerSoulId ? `&viewer_soul_id=${encodeURIComponent(viewerSoulId)}` : ''}`),
  projectRelationshipPromiseNPCKnowledge: (payload: { subject_entity_type: string; subject_entity_id: string; culture?: string; location?: string; era_context?: string; social_role?: string; access_to_archives?: boolean; education_level?: string; local_tradition?: boolean; direct_relationship?: boolean; secrecy_aware?: boolean }) => request<{ knowledge: RelationshipPromiseKnowledgeProjection }>('/api/v1/relationships/npc-knowledge', { method: 'POST', body: JSON.stringify(payload) }),

  // Phase 20: Multi-Aspect Campaign Sessions & Wandering Foundation
  listCampaignAspects: (campaignId: string) => request<{ campaign_id: string; aspects: CampaignAspect[] }>(`/api/v1/campaign/${encodeURIComponent(campaignId)}/aspects`),
  registerCampaignAspect: (payload: { campaign_id: string; soul_id: string; display_name?: string; aspect_id?: string; viewpoint_location?: string }) => request<{ aspect: CampaignAspect }>('/api/v1/campaign/aspects/register', { method: 'POST', body: JSON.stringify(payload) }),
  switchCampaignAspect: (payload: { campaign_id: string; session_id: string; target_soul_id: string }) => request<{ switch: AspectSwitchResult; active_aspect: string }>('/api/v1/campaign/aspects/switch', { method: 'POST', body: JSON.stringify(payload) }),
  getCampaignAspectView: (sessionId: string) => request<{ session_id: string; active_aspect: string; knowledge_projection: AspectKnowledgeProjection; view: Record<string, any> }>(`/api/v1/campaign/session/${encodeURIComponent(sessionId)}/aspect-view`),
  resolveCrossAspectEncounter: (payload: { session_id: string; other_soul_id: string }) => request<CrossAspectMeeting>('/api/v1/campaign/encounters/cross-aspect', { method: 'POST', body: JSON.stringify(payload) }),

  createPlace: (payload: { place_name: string; place_kind?: string; public_label?: string; region_id?: string; region_precision?: string; coordinate_latitude?: number; coordinate_longitude?: number; coordinate_precision_m?: string; consent_scope?: string; safety_status?: string; created_by_soul_id?: string }) => request<{ place: Place }>('/api/v1/wandering/places', { method: 'POST', body: JSON.stringify(payload) }),
  listPlaces: (viewerSoulId?: string, debug = false) => request<{ places: Place[] }>(`/api/v1/wandering/places${viewerSoulId ? `?viewer_soul_id=${encodeURIComponent(viewerSoulId)}${debug ? '&debug=true' : ''}` : debug ? '?debug=true' : ''}`),
  retirePlace: (payload: { place_id: string; safety_status?: string }) => request<{ place: Place }>('/api/v1/wandering/places/retire', { method: 'POST', body: JSON.stringify(payload) }),
  updateLocationConsent: (payload: { soul_id: string; location_access_granted: boolean; purpose?: string; precision_level?: string; retention_days?: number }) => request<{ consent: LocationConsent }>('/api/v1/wandering/location-consent', { method: 'POST', body: JSON.stringify(payload) }),
  getLocationConsent: (soulId: string) => request<{ consent: LocationConsent }>(`/api/v1/wandering/location-consent/${encodeURIComponent(soulId)}`),
  submitLocationSample: (payload: { soul_id: string; latitude: number; longitude: number; precision_m?: number; purpose?: string }) => request<{ sample: Record<string, any> }>('/api/v1/wandering/location-sample', { method: 'POST', body: JSON.stringify(payload) }),
  getNearbyDiscoveries: (params: { soul_id: string; latitude: number; longitude: number; radius_m?: number }) => request<NearbyDiscoveryResult>(`/api/v1/wandering/nearby?soul_id=${encodeURIComponent(params.soul_id)}&latitude=${encodeURIComponent(String(params.latitude))}&longitude=${encodeURIComponent(String(params.longitude))}${params.radius_m !== undefined ? `&radius_m=${encodeURIComponent(String(params.radius_m))}` : ''}`),
  recordWanderingDiscovery: (payload: { soul_id: string; place_id: string }) => request<{ discovery: Record<string, any>; place: Place }>('/api/v1/wandering/discoveries/record', { method: 'POST', body: JSON.stringify(payload) }),
  recordPlaceHistory: (payload: { place_id: string; event_type: string; event_id: string; soul_id?: string; provenance_source_type: string; provenance_source_id: string; visibility?: string }) => request<{ history: PlaceHistoryEntry }>('/api/v1/wandering/place-history', { method: 'POST', body: JSON.stringify(payload) }),
  getPlaceHistory: (placeId: string, viewerSoulId?: string) => request<{ place: Place; history: PlaceHistoryEntry[] }>(`/api/v1/wandering/places/${encodeURIComponent(placeId)}/history${viewerSoulId ? `?viewer_soul_id=${encodeURIComponent(viewerSoulId)}` : ''}`),

  // Living Visual World
  getLivingVisualProviderStatus: () => request<Record<string, any>>('/api/v1/living-visuals/providers/status'),
  listArtMoments: (sessionId?: string) => request<{ art_moments: ArtMoment[] }>(`/api/v1/living-visuals/art-moments${sessionId ? `?session_id=${encodeURIComponent(sessionId)}` : ''}`),
  getArtMoment: (artMomentId: string) => request<{ art_moment: ArtMoment }>(`/api/v1/living-visuals/art-moments/${encodeURIComponent(artMomentId)}`),
  evaluateArtMoments: (sessionId: string) => request<{ session_id: string; art_moments: ArtMoment[]; jobs: VisualJob[]; created: number }>('/api/v1/living-visuals/art-moments/evaluate', { method: 'POST', body: JSON.stringify({ session_id: sessionId }) }),
  curateArtMoment: (artMomentId: string, payload: { contributor_id: string; contributor_name?: string; style_guidance?: string; composition?: string; mood?: string; motif?: string; symbolism?: string; provenance_note?: string }) => request<{ curation: HumanCuration; art_moment: ArtMoment }>(`/api/v1/living-visuals/art-moments/${encodeURIComponent(artMomentId)}/curate`, { method: 'POST', body: JSON.stringify(payload) }),
  listVisualJobs: (params?: { session_id?: string; art_moment_id?: string }) => request<{ jobs: VisualJob[] }>(`/api/v1/living-visuals/jobs${params ? `?${new URLSearchParams(params as Record<string, string>).toString()}` : ''}`),
  getVisualJob: (jobId: string) => request<{ job: VisualJob }>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}`),
  processVisualJob: (jobId: string, payload?: { provider_type?: string; seed?: number }) => request<{ job: VisualJob; idempotent?: boolean }>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/process`, { method: 'POST', body: JSON.stringify(payload ?? {}) }),
  approveVisualJob: (jobId: string) => request<{ job: VisualJob }>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/approve`, { method: 'POST' }),
  rejectVisualJob: (jobId: string) => request<{ job: VisualJob }>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/reject`, { method: 'POST' }),
  hideVisualJob: (jobId: string) => request<{ job: VisualJob }>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/hide`, { method: 'POST' }),
  regenerateVisualJob: (jobId: string) => request<{ job: VisualJob }>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/regenerate`, { method: 'POST' }),
  inspectVisualSceneSpec: (jobId: string) => request<Record<string, any>>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/scene-spec`),
  getVisualGuardianVerdict: (jobId: string) => request<Record<string, any>>(`/api/v1/living-visuals/jobs/${encodeURIComponent(jobId)}/guardian`),
  getLivingVisualHistory: (params?: { entity_type?: string; entity_id?: string; soul_id?: string; session_id?: string }) => request<{ visual_history: VisualJob[] }>(`/api/v1/living-visuals/history${params ? `?${new URLSearchParams(params as Record<string, string>).toString()}` : ''}`),

  // Phase 22: Temporal Pacing & Living Time.
  getTemporalContext: (sessionId: string) => request<{ session_id: string; active_aspect: string; temporal_context: TemporalContext; campaign_time_settings: CampaignTimeSettings; aspect_temporal_state: Record<string, any>; time_source: string; offline: boolean }>(`/api/v1/campaign/session/${encodeURIComponent(sessionId)}/temporal-context`),
  evaluateTemporalEligibility: (sessionId: string) => request<TemporalEligibility>('/api/v1/campaign/time/evaluate', { method: 'POST', body: JSON.stringify({ session_id: sessionId }) }),
  advanceFictionalTime: (payload: { campaign_id: string; delta_seconds?: number; to_iso?: string }) => request<{ campaign_id: string; settings: CampaignTimeSettings; source: string }>('/api/v1/campaign/time/advance', { method: 'POST', body: JSON.stringify(payload) }),
  scheduleConsequence: (payload: { session_id: string; source_type: string; source_id: string; rule: string; eligible_after_iso?: string; min_elapsed_seconds?: number; min_events?: number; note?: string }) => request<ScheduledConsequence>('/api/v1/campaign/scheduled-consequences', { method: 'POST', body: JSON.stringify(payload) }),
  listScheduledConsequences: (sessionId?: string) => request<{ scheduled_consequences: ScheduledConsequence[] }>(`/api/v1/campaign/scheduled-consequences${sessionId ? `?session_id=${encodeURIComponent(sessionId)}` : ''}`),
  retireScheduledConsequence: (consequenceId: string) => request<{ consequence: ScheduledConsequence; idempotent: boolean }>(`/api/v1/campaign/scheduled-consequences/${encodeURIComponent(consequenceId)}/retire`, { method: 'POST' }),
  getResumeRecap: (sessionId: string) => request<ReturnRecap>(`/api/v1/campaign/session/${encodeURIComponent(sessionId)}/resume-recap`, { method: 'POST' }),
};
